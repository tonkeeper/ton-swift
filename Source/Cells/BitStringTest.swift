...
final class BitStringTest: XCTestCase {
    ...
    func testBitString() throws {
        ...
        XCTAssertFalse(try bs.at(index: 3))
        XCTAssertTrue(try bs.at(index: 4))
        XCTAssertFalse(try bs.at(index: 5))
        XCTAssertTrue(try bs.at(index: 6))
        XCTAssertFalse(try bs.at(index: 7))
        ...
        XCTAssertEqual(a, c)
        XCTAssertEqual(c, a)
        XCTAssertEqual(try a.toString(), "AA")
        XCTAssertEqual(try b.toString(), "AA")
        ...
        for c in cases {
            ...
            for i in 0..<c[0].count {
                XCTAssertEqual(try r.at(index: i), c[0][i] == "1")
            }
            
            // Check to string
            XCTAssertEqual(try r.toString(), c[1])
        }
    }
}
